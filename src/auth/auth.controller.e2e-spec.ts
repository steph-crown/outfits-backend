...
describe('AuthController (e2e)', () => {
  ...
  describe('/auth/profile (PUT)', () => {
    ...
    it('should not allow updating protected fields', async () => {
      ...
      // Profile should update successfully but protected fields should be ignored
      TestUtils.expectSuccessResponse(response, 200);
      expect(response.body.data.user.email).toBe(testUser.email); // Should not change
      expect(response.body.data.user.is_admin).toBe(false); // Should not change
    });
